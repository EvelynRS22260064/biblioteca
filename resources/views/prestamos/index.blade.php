...
@section('page-title', '📜 La Gran Bitácora')
@section('page-description', 'Supervisa el movimiento de ejemplares entre los habitantes del claro encantado')
        ...
        <div class="mb-8 flex justify-end">
            <a href="{{ route('prestamos.create') }}" 
               class="group relative bg-[#3f7847] hover:bg-[#4c8f55] text-white px-8 py-4 rounded-full font-story flex items-center gap-3 border-2 border-[#9bcf98] shadow-xl transition-all hover:shadow-2xl hover:-translate-y-1">
                ...
                <span class="text-lg">Iniciar nuevo viaje</span>
        ...
        <div class="form-card overflow-hidden border-2 border-[#8bb682] shadow-2xl relative">
            ...
            <div class="p-6 border-b border-[#99bF8c] bg-gradient-to-r from-[#f0f7e8] to-[#e5f0db]">
                <h2 class="font-story text-2xl md:text-3xl font-bold text-[#1a4524] flex items-center gap-3">
                    <i class="fa-solid fa-scroll text-[#3f7847] text-3xl"></i> 
                    <span>Los Viajes Registrados</span>
                </h2>
                <p class="text-[#34633e] text-sm md:text-base mt-2 flex items-center gap-2">
                    <i class="fa-solid fa-feather text-[#8bb682]"></i>
                    Todos los grimorios que han emprendido viaje desde nuestro claro
                </p>
            </div>
                        ...
                        <tr class="hover:bg-[#e5f0db] transition group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-[#2d5a36]">{{ $prestamo->usuario->name ?? 'N/A' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1">
                                    <i class="fa-solid fa-book text-[#8bb682] text-xs"></i>
                                    <span class="font-medium text-[#1f4a2a]">{{ $prestamo->libro->nombre ?? 'N/A' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1">
                                    <i class="fa-solid fa-feather text-[#8bb682] text-xs"></i>
                                    <span class="text-[#2d5a36]">{{ \Carbon\Carbon::parse($prestamo->fecha_prestamo)->format('d/m/Y H:i') }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if($prestamo->fecha_devolucion)
                                    <span class="flex items-center gap-1 text-[#3f7847]">
                                ...
                                @else
                                    <span class="flex items-center gap-1 text-[#b8860b] font-bold">
                                        <i class="fa-solid fa-hourglass-half"></i> En viaje
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2 flex-wrap">
                                    ...
                                    <a href="{{ route('prestamos.edit', $prestamo->id) }}" 
                                       class="btn-edit group/edit flex items-center gap-1 px-2 py-1 rounded-full hover:bg-white/50 transition">
                                    ...
                                    <!-- Devolver -->
                                    @if(!$prestamo->fecha_devolucion)
                                    <form action="{{ route('prestamos.devolver', $prestamo->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" 
                                                class="text-[#b8860b] hover:text-[#8b5a2b] transition flex items-center gap-1 px-2 py-1 rounded-full hover:bg-white/50"
                                                onclick="return confirm('¿Marcar este grimorio como devuelto al claro?')">
                                    ...
                                    <form action="{{ route('prestamos.destroy', $prestamo->id) }}" method="POST" class="inline" 
                                          onsubmit="return confirm('¿Eliminar este registro de viaje? Esta acción no se puede deshacer.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                                class="text-[#b85c5c] hover:text-[#a04545] transition flex items-center gap-1 px-2 py-1 rounded-full hover:bg-white/50">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-4 text-[#8bb682]">
                                    ...
                                    <p class="font-story text-2xl">El claro está en calma...</p>
                                    <p class="text-sm">No hay libros viajeros registrados aún</p>
                                    <a href="{{ route('prestamos.create') }}" class="mt-2 text-[#3f7847] hover:text-[#1f4a2a] underline decoration-dotted flex items-center gap-2">
            ...
            <div class="p-6 border-t border-[#99bF8c] bg-gradient-to-b from-[#fafff2] to-[#e5f0db]">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        @else
                            <a href="{{ $prestamos->previousPageUrl() }}" class="w-10 h-10 rounded-full border-2 border-[#8bb682] text-[#1f4a2a] hover:bg-[#3f7847] hover:text-white hover:border-[#3f7847] transition-all flex items-center justify-center bg-white shadow-md hover:shadow-xl">
                                <i class="fa-solid fa-chevron-left"></i>
                            </a>
                        @endif

                        @foreach(range(1, $prestamos->lastPage()) as $page)
                            ...
                            @else
                                <a href="{{ $prestamos->url($page) }}" class="w-10 h-10 rounded-full border-2 border-[#8bb682] text-[#1f4a2a] hover:bg-[#e5f0db] hover:border-[#3f7847] transition-all flex items-center justify-center font-story bg-white hover:shadow-md">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
                        ...
                        @if($prestamos->hasMorePages())
                            <a href="{{ $prestamos->nextPageUrl() }}" class="w-10 h-10 rounded-full border-2 border-[#8bb682] text-[#1f4a2a] hover:bg-[#3f7847] hover:text-white hover:border-[#3f7847] transition-all flex items-center justify-center bg-white shadow-md hover:shadow-xl">
